...
function loadProgressData() {
    let progressData = JSON.parse(localStorage.getItem('paper2Progress')) || {
        overallProgress: 0,
        topics: {
            statistics: { completed: 0, total: 14, progress: 0 },
            analyticalGeometry: { completed: 0, total: 16, progress: 0 },
            trigonometry: { completed: 0, total: 18, progress: 0 },
            euclideanGeometry: { completed: 0, total: 12, progress: 0 }
        }
    };

    // Save initial data if not exists
    if (!localStorage.getItem('paper2Progress')) {
        localStorage.setItem('paper2Progress', JSON.stringify(progressData));
    }
...
function getTopicKeyByIndex(index) {
    ...
    return topicMap[index] || 'statistics';
}
...
function selectTopic(topicIndex) {
    ...
    const topicColors = ['orange', 'purple', 'teal', 'green'];
    ...
    if (progressElement) {
        ...
        style.textContent = `
            @keyframes progressPulse {
                0% { transform: scale(1); }
                50% { transform: scale(1.2); box-shadow: 0 0 20px rgba(202, 92, 29, 0.5); }
...
document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
        ...
        if (topicCards[nextIndex]) {
            ...
            // Add visual feedback
            topicCards[nextIndex].style.boxShadow = '0 0 0 2px var(--accent-orange)';
            setTimeout(() => {
